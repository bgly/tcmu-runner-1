...
static ssize_t tcmu_glfs_read(struct tcmu_device *dev, struct tcmulib_cmd *cmd,
...
static ssize_t tcmu_glfs_write(struct tcmu_device *dev, struct tcmulib_cmd *cmd,
			       struct iovec *iov, size_t iov_cnt, size_t length,
			       off_t offset)
{
	struct glfs_state *state = tcmu_get_dev_private(dev);
